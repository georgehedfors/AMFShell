...
class RemotingService(object):
    ...
    def __init__(self, url, amf_version=pyamf.AMF0, client_type=DEFAULT_CLIENT_TYPE,
        ...
        self.strict = strict
        self.proxy = proxy
        self._setUrl(url,proxy)

    def _setUrl(self, url, proxy):
        """
        ...
        if self.url[0] == 'http':
            ...
            ##JROSE##
            if self.proxy:
                self.connection = httplib.HTTPConnection('localhost', self.proxy.split(':')[0])
            else:
            ...
            if self.proxy:
                self.connection = httplib.HTTPSConnection('localhost', self.proxy.split(':')[0])
            else:
    ...
    def _getResponse(self):
        ...
        if remoting.APPEND_TO_GATEWAY_URL in response.headers:
            ...
            self._setUrl(self.original_url, self.proxy)
